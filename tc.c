...
// Symbols
enum {
    DO_SYM, ELSE_SYM, IF_SYM, WHILE_SYM, VOID_SYM, INT_SYM, //  0->5
    LBRA, RBRA, LPAR, RPAR,                                 //  6->9
    PLUS, MINUS, STAR, SLASH, LESS, GRT, SEMI, EQUAL,       // 10->17
    INT, ID, EOI, FUNC_SYM, EQU                             // 18->22
};
...
node *test() {
    ...
    if (sym == EQU) { next_sym(); return gen(EQU, x, sum()); }
    return x;
}
